...
export default function SentinelNode({ id, data }) {
  ...
  const execute = () => {
    ...
    evaluations.push({
      ...
      score: 8.5,
      notes: 'Strong environmental considerations'
    });
    evaluations.push({
      criterion: 'Feasibility',
      score: 7.2,
      notes: 'Implementation challenges identified'
    });

    updateNodeData(id, { output: { evaluations } });
  };
    ...
    <div className={`bg-white rounded-lg shadow-lg border-2 p-4 w-80 ${getStatusColor()}`}>
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <h3 className="text-lg font-bold text-red-600">{data.role}</h3>
      ...
      {/* Anatomy Bars - Showing precision as primary */}
        ...
        {isEditing ? (
          <div className="space-y-2">
            <textarea
              ...
              className="w-full text-sm border rounded p-2 resize-none focus:ring-2 focus:ring-red-500"
              rows={4}
            />
            <select
